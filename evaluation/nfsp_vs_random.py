...
"""
    ...
    nfsp_random_bots = [nfsp_bot_0, random_bot_1]
    random_nfsp_bots = [random_bot_0, nfsp_bot_1]
    ...
    nvc_nfsp_win_rates = []
    nvc_random_win_rates = []
    nvc_draw_rates = []

    cvn_nfsp_win_rates = []
    cvn_random_win_rates = []
    cvn_draw_rates = []
    ...
    for iteration in range(num_iterations):
        ...
        all_returns_nfsp_random = play_games(env.game, nfsp_random_bots, num_games)
        all_returns_random_nfsp = play_games(env.game, random_nfsp_bots, num_games)

        # Count wins
        nvc_nfsp_wins = sum(1 for r in all_returns_nfsp_random if r[0] > r[1])
        nvc_random_wins = sum(1 for r in all_returns_nfsp_random if r[1] > r[0])
        nvc_draws = sum(1 for r in all_returns_nfsp_random if r[0] == r[1])

        cvn_random_wins = sum(1 for r in all_returns_random_nfsp if r[0] > r[1])
        cvn_nfsp_wins = sum(1 for r in all_returns_random_nfsp if r[1] > r[0])
        cvn_draws = sum(1 for r in all_returns_random_nfsp if r[0] == r[1])

        # Calculate win rates
        nvc_nfsp_win_rate = nvc_nfsp_wins / num_games * 100
        nvc_random_win_rate = nvc_random_wins / num_games * 100
        nvc_draw_rate = nvc_draws / num_games * 100

        nvc_nfsp_win_rates.append(nvc_nfsp_win_rate)
        nvc_random_win_rates.append(nvc_random_win_rate)
        nvc_draw_rates.append(nvc_draw_rate)

        cvn_nfsp_win_rate = cvn_nfsp_wins / num_games * 100
        cvn_random_win_rate = cvn_random_wins / num_games * 100
        cvn_draw_rate = cvn_draws / num_games * 100

        cvn_nfsp_win_rates.append(cvn_nfsp_win_rate)
        cvn_random_win_rates.append(cvn_random_win_rate)
        ...
        log_message = f"""
NFSP vs random results after {num_games*(iteration+1)} games:
NFSP wins: {nvc_nfsp_wins} ({nvc_nfsp_wins/num_games*100:.1f}%)
random wins: {nvc_random_wins} ({nvc_random_wins/num_games*100:.1f}%)
Draws: {nvc_draws} ({nvc_draws/num_games*100:.1f}%)
----------------------------------------------------------------
random vs NFSP results after {num_games*(iteration+1)} games:
NFSP wins: {cvn_nfsp_wins} ({cvn_nfsp_wins/num_games*100:.1f}%)
random wins: {cvn_random_wins} ({cvn_random_wins/num_games*100:.1f}%)
Draws: {cvn_draws} ({cvn_draws/num_games*100:.1f}%)

Time elapsed: {minutes} mins and {seconds:02d} secs
"""
        write_to_log("/home/ethanso/CS175/comparisons/nfsp_vs_random_1", log_message) # run_6 fixes potential flipped player ids 
    ...
    plt.plot(x, nvc_random_win_rates, label='random')
    plt.plot(x, nvc_draw_rates, label='Draws')
    plt.xlabel('Iteration')
    plt.ylabel('Win Rate (%)')
    plt.title('NFSP vs random Performance Over Iterations')
    plt.legend()
    plt.grid(True)
    ...
    plt.plot(x, cvn_random_win_rates, label='random')
    plt.plot(x, cvn_draw_rates, label='Draws')
    plt.xlabel('Iteration')
    plt.ylabel('Win Rate (%)')
    plt.title('random vs NFSP Performance Over Iterations')
    plt.legend()
    plt.grid(True)
    ...
    # Save the plot
    plt.savefig('/home/ethanso/CS175/comparisons/nfsp_vs_random_1/performance_graph.png')
    plt.close()
