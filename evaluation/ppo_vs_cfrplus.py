...
"""
    ...
    ppo_cfr_bots = [ppo_bot_0, cfr_bot_1]
    cfr_ppo_bots = [cfr_bot_0, ppo_bot_1]
    ...
    for iteration in range(num_iterations):
        ...
        all_returns_PPO_cfr = play_games(env.game, ppo_cfr_bots, num_games)
        all_returns_cfr_PPO = play_games(env.game, cfr_ppo_bots, num_games)

        # Count wins
        pvc_ppo_wins = sum(1 for r in all_returns_PPO_cfr if r[0] > r[1])
        pvc_cfr_wins = sum(1 for r in all_returns_PPO_cfr if r[1] > r[0])
        pvc_draws = sum(1 for r in all_returns_PPO_cfr if r[0] == r[1])

        cvp_cfr_wins = sum(1 for r in all_returns_cfr_PPO if r[0] > r[1])
        cvp_ppo_wins = sum(1 for r in all_returns_cfr_PPO if r[1] > r[0])
        cvp_draws = sum(1 for r in all_returns_cfr_PPO if r[0] == r[1])

        # Calculate win rates
        pvc_ppo_win_rate = pvc_ppo_wins / num_games * 100
        pvc_cfr_win_rate = pvc_cfr_wins / num_games * 100
        pvc_draw_rate = pvc_draws / num_games * 100

        pvc_ppo_win_rates.append(pvc_ppo_win_rate)
        pvc_cfr_win_rates.append(pvc_cfr_win_rate)
        pvc_draw_rates.append(pvc_draw_rate)

        cvp_ppo_win_rate = cvp_ppo_wins / num_games * 100
        cvp_cfr_win_rate = cvp_cfr_wins / num_games * 100
        cvp_draw_rate = cvp_draws / num_games * 100

        cvp_ppo_win_rates.append(cvp_ppo_win_rate)
        cvp_cfr_win_rates.append(cvp_cfr_win_rate)
        ...
        log_message = f"""
----------------------------------------------------------------
PPO vs CFRPlus results after {num_games*(iteration+1)} games:
PPO wins: {pvc_ppo_wins} ({pvc_ppo_wins/num_games*100:.1f}%)
CFR+ wins: {pvc_cfr_wins} ({pvc_cfr_wins/num_games*100:.1f}%)
Draws: {pvc_draws} ({pvc_draws/num_games*100:.1f}%)

CFRPlus vs PPO results after {num_games*(iteration+1)} games:
PPO wins: {cvp_ppo_wins} ({cvp_ppo_wins/num_games*100:.1f}%)
CFR+ wins: {cvp_cfr_wins} ({cvp_cfr_wins/num_games*100:.1f}%)
Draws: {cvp_draws} ({cvp_draws/num_games*100:.1f}%)

Time elapsed: {minutes} mins and {seconds:02d} secs
----------------------------------------------------------------
"""
        write_to_log("/home/ethanso/CS175/comparisons/run_9", log_message) # run_6 fixes potential flipped player ids 
    ...
    plt.plot(x, pvc_cfr_win_rates, label='CFR+')
    plt.plot(x, pvc_draw_rates, label='Draws')
    plt.xlabel('Iteration')
    plt.ylabel('Win Rate (%)')
    plt.title('PPO vs CFR+ Performance Over Iterations')
    plt.legend()
    plt.grid(True)
    ...
    plt.plot(x, cvp_cfr_win_rates, label='CFR+')
    plt.plot(x, cvp_draw_rates, label='Draws')
    plt.xlabel('Iteration')
    plt.ylabel('Win Rate (%)')
    plt.title('CFR+ vs PPO Performance Over Iterations')
    plt.legend()
    plt.grid(True)
    ...
    # Save the plot
    plt.savefig('/home/ethanso/CS175/comparisons/run_9/performance_graph.png')
    plt.close()
